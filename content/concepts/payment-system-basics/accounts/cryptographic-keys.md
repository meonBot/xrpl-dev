...
=
[![Pas[![Passphrase → Seed → Private Key → Prefix + Public Key](img/key-derivation-ed25519.png)](img/key-derivation-ed25519.png)
>>>>>>> 4f6579f
...
<<<<<<< master
4. De4. Derive the master public key pair by adding the intermediate public key to the root public key. Similarly, derive the secret key by adding the intermediate secret key to the root secret key.
...
>>>>>>>+master
SA priva    - An ECDSA private key is just a very large integer, so you can calculate the sum of two private keys by summing them modulo the secp256k1 modulus.

    - An ECDSA public key is a point on the elliptic curve, so you should use elliptic curve math to sum the points.
>>>>>>>+4f6579f
ic key is a point on the elliptic curve, so you should use elliptic curve math to sum the points.
